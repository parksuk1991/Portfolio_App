...
# yfinance 임포트를 try-except로 감싸서 에러 처리
try:
    import yfinance as yf
except ImportError:
    st.error("yfinance 모듈을 찾을 수 없습니다. requirements.txt 파일을 확인해주세요.")
    st.stop()
...
# 메인 앱
def main():
    st.title("📈 Portfolio Backtesting App by Chris S. Park")
    ...
    **이 앱은 데이터 공백 자동 보완 기능을 갖춘 모멘텀 기반 포트폴리오 백테스팅 도구입니다.**
    ...
    - **데이터 공백 자동 보완**: 선택한 자산의 과거 데이터가 부족한 경우, 유사한 특성을 가진 대체 자산으로 자동 보완
    ...
    #### 🔧 데이터 보완 방식
    - **유사 자산 매핑**: 새로운 ETF를 과거에 존재했던 유사한 성격의 자산으로 대체
